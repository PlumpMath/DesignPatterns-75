    ...
    public partial class FormMain : Form
    {
        ...
        private void fetchPhotosFromSpecificAlbum(object sender)
        {
            ...
            foreach (Photo photoFromAlbum in currentAlbum.Album.Photos)
            {
                ...
                thePictureBoxPhotoIWant.Location = new Point(x, y);
                thePictureBoxPhotoIWant.Size = new Size(200, 200);
                thePictureBoxPhotoIWant.SizeMode = PictureBoxSizeMode.StretchImage;
                thePictureBoxPhotoIWant.LoadAsync(photoFromAlbum.URL);

                thePictureBoxPhotoIWant.MouseEnter += (object send, EventArgs e) =>
                {
                    this.enlargePhotoSizeOnMouseHover(send);
                };

                thePictureBoxPhotoIWant.MouseLeave += (object send, EventArgs e) =>
                {
                    this.reducePhotoSizeOnMouseLeave(send); 
                };

                formPhotosFromAlbum.Controls.Add(thePictureBoxPhotoIWant);
                x += 220;
                xCounter--;
                if (xCounter == 0)
                {
