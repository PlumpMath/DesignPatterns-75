...
namespace Ex01.DesktopFacebook
{
    public partial class FormMain : Form
    {
        ...
        private void fetchEvents()
        {
            ...
            formUserEvents.m_ReportWhenEventHappening += this.executeAnnouncement;
        ...
        private void executeAnnouncement()
        {
            SystemSounds.Hand.Play();
            MessageBox.Show("Event happening now!");
        }
        ...
        private void fetchAlbums()
        {
            ...
            foreach (Album userAlbum in this.m_LoggedInUser.Albums)
            {
                ...
                {
                    MyPictureBoxAlbum thePictureBoxAlbumIWant = MyPictureBoxGeneral.Create(MyPictureBoxGeneral.e_PictureType.Album) as MyPictureBoxAlbum;
                    thePictureBoxAlbumIWant.Album = userAlbum;
                    Label albumNameLabel = new Label();

                    thePictureBoxAlbumIWant.Location = new Point(x, y);
                    thePictureBoxAlbumIWant.Size = new Size(150, 150);
                    thePictureBoxAlbumIWant.SizeMode = PictureBoxSizeMode.StretchImage;
                    thePictureBoxAlbumIWant.LoadAsync(userAlbum.CoverPhoto.URL);
                    thePictureBoxAlbumIWant.Click += (object sender, EventArgs e) =>
                    {
                        this.fetchPhotosFromSpecificAlbum(sender);
                    };

                    thePictureBoxAlbumIWant.BorderStyle = BorderStyle.Fixed3D;
                    albumNameLabel.AutoSize = false;
                    albumNameLabel.Size = new Size(150, 20);
                    albumNameLabel.Text = string.Format(@"Album Name: {0}", thePictureBoxAlbumIWant.Album.Name);
                    albumNameLabel.Location = new Point(thePictureBoxAlbumIWant.Left, thePictureBoxAlbumIWant.Bottom + 2);
                    ...
                    formUserAlbums.Controls.Add(thePictureBoxAlbumIWant);
        ...
        private void fetchPhotosFromSpecificAlbum(object sender)
        {
            ...
            List<MyPictureBoxGeneral> collectionOfPictureBoxPhotos = new List<MyPictureBoxGeneral>();

            foreach (Photo photoFromAlbum in currentAlbum.Album.Photos)
            {
                MyPictureBoxPhoto thePictureBoxPhotoIWant = MyPictureBoxGeneral.Create(MyPictureBoxGeneral.e_PictureType.Photo) as MyPictureBoxPhoto;
                thePictureBoxPhotoIWant.Photo = photoFromAlbum;
                collectionOfPictureBoxPhotos.Add(thePictureBoxPhotoIWant);
            }
            
            int j = 0;
            foreach (MyPictureBoxGeneral picture in collectionOfPictureBoxPhotos)
            {
                if (j + 1 < collectionOfPictureBoxPhotos.Count)
                {
                    collectionOfPictureBoxPhotos.ElementAt<MyPictureBoxGeneral>(j).SetNext(collectionOfPictureBoxPhotos.ElementAt<MyPictureBoxGeneral>(j + 1));
                }
            ...
            slideShow.PictureBoxToSet = collectionOfPictureBoxPhotos.ElementAt<MyPictureBoxGeneral>(0);
        ...
        {
            FormEmailManager formEmail = new FormEmailManager();
